...
export const App = ({ isServerInfo }: { isServerInfo: any }) => {
  ...
  return (
    <BrowserRouter>
      ...
      {unsupportedNetwork ? (
        ...
        <div
          className={`md:mr-[30px] pt-[75px] h-full transition-all duration-500 ${
            isSidebarMinimized ? 'md:ml-[96px] ' : 'md:ml-[230px] '
          }`}
        >
          {showBanner && <MarketingBanner />}
          <main
            className={`max-w-[1648px] mx-auto pb-30 h-full pl-20 pr-20 ${
              showBanner ? 'pt-40 md:pt-20' : 'pt-20'
            }`}
          >
            <Router isServerInfo={isServerInfo} />
          </main>
        </div>
      )}
      <NotificationAlerts />
