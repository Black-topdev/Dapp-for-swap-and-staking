...
const menu: SecondaryMenuItem[] = [
  ...
  },
  {
    ...
    subMenu: [
      {
        label: 'Docs',
        to: 'https://docs.thalesia.com/',
        icon: <IconDocument />,
      },
      {
        label: 'GitHub',
        to: 'https://github.com/thalesia/',
        icon: <IconGithub />,
      },
    ],
  },
            ...
            <Popover key={index} className="relative">
              ...
              {indexDesktop === index && item.subMenu && (
                <Popover.Panel
                  ...
                  className={`absolute w-[225px] py-20 bg-black bottom-[-18px] px-24 left-0 rounded z-10 transition ease-out duration-500 ${
                    isMinimized ? 'ml-[66px]' : 'ml-[200px]'
                  } ${indexDesktop === index ? 'opacity-100' : 'opacity-0'}`}
                >
                  <div className="absolute h-14 w-14 transform bottom-24 left-[-7px] rotate-45 bg-black" />
